                ...
                if (i == size - 1 || mS.coeff(i + 1, i) == Scalar(0)) {
                        ...
                        if (abs(m_betas.coeffRef(i)) >= (std::numeric_limits<RealScalar>::min)()) {
                            ...
                            for (Index j = i - 1; j >= 0; j--) {
                                ...
                                if (j > 0 && mS.coeff(j, j - 1) != Scalar(0)) {
                                    // 2x2 block
                                    Matrix<Scalar, 2,
                                           1> rhs = (alpha * mT.template block<2, Dynamic>(j - 1, st, 2, sz) - beta * mS.template block<2, Dynamic>(j - 1, st, 2,
                                                                                                                                                    sz)).lazyProduct(v.segment(st,
                                                                                                                                                                               sz));
                                    Matrix<Scalar, 2, 2> lhs = beta * mS.template block<2, 2>(j - 1, j - 1) - alpha * mT.template block<2, 2>(j - 1, j - 1);
                                    v.template segment<2>(j - 1) = lhs.partialPivLu().solve(rhs);
                                    j--;
                                } else {
                                    v.coeffRef(j) = -v.segment(st,
                                                               sz).transpose().cwiseProduct(beta * mS.block(j, st, 1, sz) - alpha * mT.block(j, st, 1,
                                                                                                                                             sz)).sum() / (beta * mS.coeffRef(j,
                                                                                                                                                                              j) -
                                                                                                                                                           alpha * mT.coeffRef(j,
                                                                                                                                                                               j));
                                }
                    ...
                    if (computeEigenvectors) {
                        ...
                        for (Index j = i - 1; j >= 0; j--) {
                            ...
                            if (j > 0 && mS.coeff(j, j - 1) != Scalar(0)) {
                                // 2x2 block
                                Matrix<ComplexScalar, 2,
                                       1> rhs = (alpha * mT.template block<2, Dynamic>(j - 1, st, 2, sz) - beta * mS.template block<2, Dynamic>(j - 1, st, 2,
                                                                                                                                                sz)).lazyProduct(cv.segment(st,
                                                                                                                                                                            sz));
