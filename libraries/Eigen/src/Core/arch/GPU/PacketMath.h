...
// we'll use on the host side (SSE, AVX, ...)
        ...
        template<> struct unpacket_traits<float4>  { typedef float type;
                                                     enum { size=4, alignment=Aligned16, vectorizable=true, masked_load_available=false, masked_store_available=false };
                                                     typedef float4 half; };
        template<> struct unpacket_traits<double2> { typedef double type;
...
// of the functions, while the latter can only deal with one of them.
        ...
        template<> EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE float4 pconj(const float4& a) {
            return a;
        }
        template<> EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE double2 pconj(const double2& a) {
            return a;
        }
...
// its corresponding packet_traits<Eigen::half> must be visible on host.
        ...
        template<> struct unpacket_traits<Packet4h2> { typedef Eigen::half type;
        ...
        template<> struct unpacket_traits<half2> { typedef Eigen::half type;
                                                   enum { size=2, alignment=Aligned16, vectorizable=true, masked_load_available=false, masked_store_available=false };
                                                   typedef half2 half; };
